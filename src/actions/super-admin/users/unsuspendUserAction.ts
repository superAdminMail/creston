...
export type UnsuspendUserState = FormActionState<FieldName>;

export async function unsuspendUserAction(
  _previousState: UnsuspendUserState,
  formData: FormData,
): Promise<UnsuspendUserState> {
  ...
  if (parsedUserId.trim() === actorUserId) {
    return createErrorFormState("You cannot change your own account state.");
  }
  ...
  try {
    ...
    if (
      ...
      return createErrorFormState(
        "This account has already been deleted and cannot be reactivated.",
      );
    }
    ...
    if (
      ...
      return createSuccessFormState("Account is already active.");
    }
    ...
    return createSuccessFormState("User reactivated successfully.");
  } catch (error) {
    return createErrorFormState(
      getFriendlyServerError(
        error,
        "Unable to reactivate this user right now.",
      ),
    );
  }
}
