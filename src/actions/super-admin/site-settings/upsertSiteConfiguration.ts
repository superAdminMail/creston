...
function getFormData(formData: FormData) {
  return {
    ...
    supportPhoneSecondary: String(
      formData.get("supportPhoneSecondary") ?? "",
    ),
...
export async function upsertSiteConfiguration(
  ...
  if (!parsed.success) {
    ...
    return createErrorState("Please review the highlighted site settings.", {
      ...
      siteAddress: flattened.siteAddress?.[0],
      siteCRN: flattened.siteCRN?.[0],
      siteFRN: flattened.siteFRN?.[0],
      supportEmail: flattened.supportEmail?.[0],
      supportPhone: flattened.supportPhone?.[0],
      supportPhoneSecondary: flattened.supportPhoneSecondary?.[0],
