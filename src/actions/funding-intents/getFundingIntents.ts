...
import { CryptoFundingIntentStatus, type InvestmentOrderStatus } from "@/generated/prisma";
...
export type FundingIntentListItem = {
  id: string;
  userName: string;
  userEmail: string;
  provider: string;
...
export type FundingIntentsPageData = {
  totalIntentsCount: number;
  fundedIntentsCount: number;
  processingIntentsCount: number;
  exceptionIntentsCount: number;
  intents: FundingIntentListItem[];
};

export async function getFundingIntents(): Promise<FundingIntentsPageData> {
